...
def seson(temp, hum):
    ...
    Seson_guess_response = model.generate_content("based on the this weather data: "+str(
        temp)+", and "+ str(hum)+", and the prameters that are: if the Temperature is 30 degreese or below it is cold,  and the Humidity is high if it is above 70%,  can you tell me if it is hot or cold, and if it is humid or dry")
    import re

    def strip_html(text):
        return re.sub(r'<[^>]+>', '', text)
    season_weather = strip_html(Seson_guess_response.text.strip())
    season = None
    weather = None
    ...
    recipe_text = strip_html(Recpie_response.text)
    return season_weather, recipe_text
